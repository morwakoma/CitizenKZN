...
namespace eRecruitment.Sita.Web
{
  public class eRecruitmentDataAccess
  {
    ...
    //Add Attachments 
    public void AddAttachments(int profileID, string fileName, byte[] fileData, string contentType,string FileExtension,string documentName)
    {
      using (eRecruitment.Sita.Web.App_Data.DAL.eRecruitmentDataClassesDataContext _db = new eRecruitment.Sita.Web.App_Data.DAL.eRecruitmentDataClassesDataContext())
      {
        _db.AddAttachments(profileID, fileName, fileData, contentType, FileExtension,documentName);
      }
    }
    ...
    public AttachmentModel GetAttachment(string userID,String documentType)
    {
            var ProfileID = GetProfileID(userID);
            var attachments = _db.Attachments.Where(x => x.ProfileID == ProfileID && x.DocumentType == documentType);
            AttachmentModel attachmentResult = null;
            foreach (var attachment in attachments)
            {

                attachmentResult = new AttachmentModel()
                {
                    attachmentID = attachment.AttachmentID,
                    contentType = attachment.contentType,
                    //createdon = Convert.ToDateTime(attachment.CreateOn),
                    fileData = attachment.fileData.ToArray(),
                    fileExtension = attachment.FileExtension,
                    fileName = attachment.fileName,
                    profileID = Convert.ToInt32(attachment.ProfileID),
    ...
    public void UpdateAttachments(int AttachmentID, string fileName, byte[] fileData, string contentType, string FileExtension,string DocumentType)
    {
      using (eRecruitment.Sita.Web.App_Data.DAL.eRecruitmentDataClassesDataContext _db = new eRecruitment.Sita.Web.App_Data.DAL.eRecruitmentDataClassesDataContext())
      {
        _db.proc_eRecruitmentUpdateAttachment(AttachmentID, fileName, fileData, contentType, FileExtension, DocumentType);
      }
    }
...
public List<ProfileViewModel> GetCandidateProfileInfo(string ID)
    ...
    public List<AttachmentModel> GetAttachmentByProfileID(string Userid)
    {
      ...
      using (eRecruitment.Sita.Web.App_Data.DAL.eRecruitmentDataClassesDataContext _db = new eRecruitment.Sita.Web.App_Data.DAL.eRecruitmentDataClassesDataContext())
      {
        ...
        var Attachment = _db.Attachments.Where(x => x.ProfileID == id).ToList();
                ...
                foreach (var d in Attachment)
        {
          ...
          e.profileID = Convert.ToInt32(d.ProfileID);
          e.fileName = Convert.ToString(d.fileName);
          e.fileData = d.fileData.ToArray();
          e.contentType = Convert.ToString(d.contentType);
          e.fileExtension = Convert.ToString(d.FileExtension);
    ...
    {
            //var attachment = _db.Attachments.Where(x => x.AttachmentID == id).FirstOrDefault();

            _db.DeleteAttachments(id);
            _db.SubmitChanges();
    }

        public VacancySalaryModel getVacancySalary(int VacancyID)
        {
            
           return (from vS in _db.tblVacancySalaries

             where vS.VacancyID == VacancyID
             select new VacancySalaryModel()
             {
                 SalaryID = vS.SalaryID,
                 JobTitleID = int.Parse(vS.JobTitleID.ToString()),
                 JobLevel = vS.JobLevel,
                 MinValue = (decimal)vS.MinValue,
                 MaxValue = (decimal)vS.MaxValue,
